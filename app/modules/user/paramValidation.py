            ...
            if rute == "/postUser":
                if "account_type" in data:
                    if data["account_type"] == "personal":
                        rute = "/postUserPersonal"
                    elif data["account_type"] == "business":
                        rute = "/postUserBusiness"
                    else:
                        raise Exception("account_type debe ser personal o business")
                else:
                    raise Exception("account_type es un parámetro requerido")
                
            #* Lista de funciones que contienen las reglas de parámetros
            listFuntion = {
                '/getUser'           : 'userGetRute',
                '/putUser'           : 'userPutRute',
                '/postUserPersonal'  : 'userPersonalPostRute',
                '/postUserBusiness'  : 'userBusinessPostRute',
                '/generated_code'    : 'userGeneratedCode',
                '/getUserAll'        : 'userAllGetRute',
            }
            ...
            if(rute in listFuntion):
                ...
                if(bool(data)):
                    for key in data:
                        ...
                        if(key not in setMerge and key not in paramsDefault):
                            key = "password" if key == "passwd" else key
